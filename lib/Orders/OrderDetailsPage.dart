        ...
        body: SingleChildScrollView(
          child: FutureBuilder<DocumentSnapshot>(
            future: MyPoultry.firestore
                ...
                .collection(MyPoultry.collectionOrders).document(orderID).get(),
            builder: (c, snapshot) {
              Map dataMap;
              if(snapshot.hasData)
                {
              ...
              return snapshot.hasData
                  ? Container(
                      child: Column(
                        children: [
                          StatusBanner(status: dataMap[MyPoultry.isSuccess],),
                          ...
                          Padding(
                            ...
                            child: Align(
                              ...
                              child: Text(
                                "Ksh " + dataMap[MyPoultry.totalAmount].toString(),
                          ...
                          FutureBuilder<DocumentSnapshot>(
                            future: MyPoultry.firestore
                                .collection(MyPoultry.collectionUser)
                                .document(MyPoultry.sharedPreferences.getString(MyPoultry.userUID))
                                .collection(MyPoultry.subCollectionAddress).document(dataMap[MyPoultry.addressID]).get(),
                            builder: (c, snap) {
                              return snap.hasData
                                  ? ShippingDetails(
...
class ShippingDetails extends StatelessWidget {
  ...
  confirmedUserOrderReceived(BuildContext context, String mOrderId) {
    MyPoultry.firestore
        .collection(MyPoultry.collectionUser)
        .document(MyPoultry.sharedPreferences.getString(MyPoultry.userUID))
        .collection(MyPoultry.collectionOrders).document(mOrderId).delete();
          ...
          child: Center(
            child: InkWell(
              ...
              child: Container(
                ...
                child: Center(
                  child: Text("Order Confirmed", style: TextStyle(color: Colors.white, fontSize: 15.0),),
                ),
              ),
            ),
          ),
